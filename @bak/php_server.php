...
class WebSocket {
    const LOG_PATH = './'; // dirname(dirname())
    ...
    public function connect($socket) {
        ...
        $this->debug(array_merge(array('socket_connect'), $socket_info));
    }
    ...
    private function debug(array $info) {
        ...
        file_put_contents(self::LOG_PATH . 'php_websocket_debug.log', implode(' | ', $info) . "\r\n", FILE_APPEND);
    }
    ...
    private function error(array $info) {
        ...
        $info = array_map('json_encode', $info);
        file_put_contents(self::LOG_PATH . 'php_websocket_error.log', implode(' | ', $info) . "\r\n", FILE_APPEND);
    }
}

$ws = new WebSocket("127.0.0.1", "8839");
